...
# =============================================================================
# SIGN-ZERO HAMMING BLEND SEARCH (ported from Membot Feb 16)
# =============================================================================

SEARCH_STOP_WORDS = {"the", "a", "an", "is", "are", "was", "were", "of", "in", "to",
                     "and", "or", "for", "on", "it", "be", "as", "at", "by", "this",
                     "that", "with", "from", "not", "but", "has", "had", "have", "do",
                     "does", "did", "will", "can", "its", "who", "what", "how", "why",
                     "me", "tell", "show", "find", "search", "about"}


def hamming_blend_search(q_emb, embeddings, texts, top_k=10, keywords=None):
    ...
    if keywords:
        ...
        for i in candidate_idx:
            ...
            boosted.append((i, base_score + boost, float(cos_scores[i]), float(ham_scores[i]), boost))

        boosted.sort(key=lambda x: x[1], reverse=True)
        return boosted[:top_k]
    else:
        top_idx = np.argsort(blended)[-top_k:][::-1]
        return [(int(i), float(blended[i]), float(cos_scores[i]), float(ham_scores[i]), 0.0) for i in top_idx]
...
with st.sidebar:
    ...
    selected_mode = st.radio("Search Mode", search_modes, index=0,
                              help="Cosine + Hamming: 70/30 blend with keyword reranking (no GPU needed). Same as Membot.")
...
if search_clicked and query and len(query) > 2:
    ...
    # --- SEARCH ---
    if selected_mode == "🔍 Cosine + Hamming Blend":
        # SIGN-ZERO HAMMING BLEND — same pipeline as Membot (no GPU needed)
        ham_keywords = [w for w in words if w not in SEARCH_STOP_WORDS]
        ham_results = hamming_blend_search(q_emb, embeddings, texts, top_k=top_k,
                                           keywords=ham_keywords if ham_keywords else None)
        search_mode = f"🔍 Cosine+Hamming 70/30"
        if ham_keywords:
            search_mode += f" +kw({','.join(ham_keywords[:3])})"
        results = []
        blend_details = {}
        for entry in ham_results:
            idx, final_score, cos_s, ham_s, kw_boost = entry
            idx = int(idx)
            results.append((final_score, idx, None))
            blend_details[idx] = (cos_s, ham_s, kw_boost)

    elif selected_mode == "🧠 Pure Brain (no pkl needed!)" and st.session_state.signatures_loaded:
            ...
            with st.container(border=True):
                ...
                with hcol2:
                    if idx in blend_details:
                        detail = blend_details[idx]
                        if len(detail) == 3 and selected_mode.startswith("🔍"):
                            cos_s, ham_s, kw_b = detail
                            label = f"{score:.3f}\nC:{cos_s:.3f} H:{ham_s:.3f}"
                            if kw_b > 0:
                                label += f" +{kw_b:.3f}"
                            st.caption(label)
                        else:
                            cos_s, phys_s = detail[0], detail[1]
