...
- **Per-seat memory** — each seat now has its own attention: what *you* have read, marked, and returned to, recorded as you search. Two people searching the same shared cart build different hierarchies over it. Attention is keyed to your account UUID rather than your email, because emails change and re-point history at the wrong person. Signed out, nothing is recorded — an anonymous visitor does not accumulate into anyone's profile.
- **Edits no longer break clusters.** Editing a passage used to be two independent actions (add the new text, delete the old) with nothing recording that they were related — so a curated cluster quietly lost that member and your attention on it vanished. There is now a single edit action that records the link, and everything that points at the old text follows the edit and *says* that it followed.
- **Stale saves are refused.** Two people with the same cart open hold two independent copies. Previously the second to save silently wrote back whatever the first had deleted. Now the save is refused, naming who changed it and by how much. It refuses rather than merges: a refused save costs you a reload, a silent one costs whatever your colleague deleted.
- **Hide** — mark a passage as noise **in your view only**. It drops out of your hot stack and your search results; nothing is deleted, nobody else is affected, and one click puts it back. This exists because the alternative — deleting shared content because it clutters *your* list — is how company knowledge gets lost.
- **Agent proposals** — agents can flag a passage as wrong, outdated, duplicated or incomplete, and suggest a correction. They cannot edit or delete anything: an agent's corpus can be poisoned into instructing it to destroy things, so the capability simply does not exist. Flags ride back with search results, and an owner decides.
...
**New to multi-seat?** [`docs/GUIDE-MULTI-SEAT.md`](../docs/GUIDE-MULTI-SEAT.md) covers the whole model in one place — seats, overlays, sub-cartridges, succession, the firm view, proposals — plus the vocabulary and an honest table of what is enforced today versus what is still coming.
